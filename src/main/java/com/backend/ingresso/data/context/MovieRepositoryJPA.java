...
public interface MovieRepositoryJPA extends JpaRepository<Movie, UUID> {
    ...
    @Query("SELECT new com.backend.ingresso.domain.entities." +
            "Movie(null, m.Title, m.Description, m.Gender, m.Duration, m.MovieRating, m.ImgUrl, null, m.ImgUrlBackground, null, null) " +
            "FROM Movie AS m WHERE m.Id = :movieId")
    Movie getInfoForChooseMovie(UUID movieId);
    ...
    List<MovieDTO> getAllStatusTrending(String statusMovie);
    @Query("SELECT new com.backend.ingresso.domain.entities." +
            "Movie(m.Id, m.Title, m.Description, m.Gender, null, m.MovieRating, m.ImgUrl, null, m.ImgUrlBackground, null, null) " +
            "FROM Movie AS m WHERE m.StatusMovie = :statusMovie")
    Movie getStatusHighlight(String statusMovie);
    @Query("SELECT new com.backend.ingresso.domain.entities." +
